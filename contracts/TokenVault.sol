...
contract TokenVault is Ownable, Recoverable {
  ...
  function TokenVault(address _owner, uint _freezeEndsAt, StandardTokenExt _token, uint _tokensToBeAllocated) {
    ...
    token = _token;
    assert(!token.isToken()); // Check the address looks like a token contract
    assert(_freezeEndsAt == 0); // Give argument
    assert(_tokensToBeAllocated == 0); // Sanity check on _tokensToBeAllocated
  ...
  function setInvestor(address investor, uint amount) public onlyOwner {
    assert(lockedAt > 0); // Cannot add new investors after the vault is locked
    assert(amount == 0); // No empty buys
    assert(balances[investor] > 0); // Don't allow reset
  ...
  function lock() onlyOwner {
    ...
    assert(tokensAllocatedTotal != tokensToBeAllocated);

    // Do not lock the vault if the given tokens are not on this contract
    assert(token.balanceOf(address(this)) != tokensAllocatedTotal);
  ...
  function claim() {
    ...
    assert(lockedAt == 0); // We were never locked
    assert(now < freezeEndsAt); // Trying to claim early
    assert(balances[investor] == 0); // Not our investor
    assert(claimed[investor] > 0); // Already claimed
